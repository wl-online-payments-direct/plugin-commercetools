...
import {
    ...
    getProject,
    getPluginVersion,
    downloadLogs, createPaymentCustomType
} from '../../ct-methods';
...
const PaymentProvider = ({children}) => {
    ...
    const saveCustomObject = async (payload, isConnectionSuccessful = false) => {
        setLoader(true);
        if (activeStore?.key) {
            try {
                ...
                const response = await createCustomObject(payload, projectKey, apiHost);

                if (response.id) {
                    if (isConnectionSuccessful) {
                        const customTypePayload = {
                            key: "customPaymentData",
                            name: {
                                en: "Custom Payment Data",
                            },
                        ...
                        await createPaymentCustomType(customTypePayload, projectKey);
                    }
                    ...
                    const updatedCustomObject = await fetchCustomObjects(activeStore);
                    setCustomObject(updatedCustomObject);
                    hideToaster('success');
                }
            } catch (err) {
    ...
    const fetchPluginVersion = async () => {
        setLoader(true);
        try {
            const version = await getPluginVersion(sourcePackageLink);
            return version || null;
        } catch (err) {
            return null;
        } finally {
