...
const OrderDetails = () => {
  ...
  const { openCapture, openRefund, openCancel, transactionRequested } =
  ...
  const columns = [
    ...
    {
      key: 'fraudServiceResult',
      label: formatMessage(messages.fraudServiceResult),
    },
    {
      key: 'authenticationStatus',
      label: formatMessage(messages.authenticationStatus),
    },
  ...
  if (openCapture) {
    ...
    return (
      <CaptureAmount
        ...
        transactionList={orderDetails.Operations?.filter(
          (operation) => operation.status === 'CAPTURED'
        )}
      />
    );
  }
  ...
  if (openRefund) {
    ...
    return (
      <RefundAmount
        ...
        transactionList={orderDetails.Operations?.filter(
          (operation) => operation.status === 'REFUNDED'
        )}
      />
    );
  }
      ...
      <PageWrapper title={'Order details'}>
        <div className="failed-order">
          ...
          <div>
            <FormattedMessage
              id="noOrders"
              defaultMessage="There are no orders for the Payment Id:"
            />
            {orderId}
          </div>
        </div>
